...
#
# Storage strategy:
#   1Password: Single "my_setup" secure note with sections (Git Personal, Git Work, Tokens, Network)
#   Config:    ~/.config/my_setup/config for non-secret machine-specific settings
...
OP_ITEM_NAME="my_setup"
OP_VAULT="Personal"
OP_TAG="managed:my_setup"
...
if [[ "$PROVIDER" == "1password" ]]; then
    ...
    if op item get "$OP_ITEM_NAME" --vault="$OP_VAULT" &>/dev/null; then
        ...
        op item create \
            ...
            --tags="$OP_TAG" \
            "${OP_FIELDS[@]}" \
            --notes="Managed by my_setup bootstrap-secrets.sh. Do not modify manually — re-run secrets/bootstrap-secrets.sh to update." \
            >/dev/null
    fi
    ...
    _pass_store "my_setup/git-personal/name" "$GIT_NAME_PERSONAL"
    _pass_store "my_setup/git-personal/email" "$GIT_EMAIL_PERSONAL"
    [[ -n "${GIT_SIGNING_KEY:-}" ]] && _pass_store "my_setup/git-personal/signing-key" "$GIT_SIGNING_KEY"

    if [[ "${has_work:-n}" == "y" ]]; then
        _pass_store "my_setup/git-work/name" "$GIT_NAME_WORK"
        _pass_store "my_setup/git-work/email" "$GIT_EMAIL_WORK"
        _pass_store "my_setup/git-work/repo-dir" "$WORK_REPO_DIR"
        [[ -n "${WORK_SIGNING_KEY:-}" ]] && _pass_store "my_setup/git-work/signing-key" "$WORK_SIGNING_KEY"
    fi

    [[ -n "${NETWORK_PASSWORD:-}" ]] && _pass_store "my_setup/network/backup-password" "$NETWORK_PASSWORD"
    log_info "Secrets stored in pass under my_setup/"
fi

# === Step 4: Write local config file ===
log_step "Step 4: Writing local config..."

CONFIG_DIR="${HOME}/.config/my_setup"
CONFIG_FILE="${CONFIG_DIR}/config"
mkdir -p "$CONFIG_DIR"

cat > "$CONFIG_FILE" << EOF
# my_setup — Machine-specific settings (non-secret)
...
if [[ "$PROVIDER" == "1password" ]]; then
    ...
    test_val=$(pass show "my_setup/git-personal/name" 2>/dev/null | head -n1 || echo "")
    if [[ -n "$test_val" ]]; then
