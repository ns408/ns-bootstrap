...
Identity & credentials (git name, email, signing keys, tokens)
  ↓
  1Password: Single "my_setup" secure note with sections
  (macOS) / pass structured paths (Ubuntu)

Machine-specific config (paths, non-secret settings)
  ↓
  ~/.config/my_setup/config (sourced by loader.sh)
...
All bootstrap-managed secrets live in a **single Secure Note** called `my_setup`:

```
1Password Vault: Personal
└── my_setup  (Secure Note, tag: managed:my_setup)
...
Read with: `op read "op://Personal/my_setup/Git Personal/name"`

The item is tagged `managed:my_setup` with a note:
> *Managed by my_setup bootstrap-secrets.sh. Do not modify manually.*

### Local Config File

Non-secret, machine-specific settings are stored in `~/.config/my_setup/config`:
...
get_secret "git/name-personal"     # → op://Personal/my_setup/Git Personal/name
get_secret "homebrew/github-token" # → op://Personal/my_setup/Tokens/homebrew_github
...
3. **Run bootstrap:**
   ...
   ./secrets/bootstrap-secrets.sh
   ```
   This creates the `my_setup` secure note with all your git config, writes `~/.config/my_setup/config`, and processes `.gitconfig` templates.
...
4. **Read secrets:**
   ...
   op read "op://Personal/my_setup/Git Personal/email"
   ```
...
op item get "my_setup" --vault=Personal

# List fields
op item get "my_setup" --vault=Personal --format=json | jq '.fields[] | {section: .section.label, label: .label}'
